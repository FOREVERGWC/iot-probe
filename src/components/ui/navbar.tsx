...
const Navbar = () => {
	...
	return (
		<NavigationMenu className="bg-gray-800 p-4">
			<NavigationMenuList>
				<NavigationMenuItem>
					<span className="text-white text-lg font-bold">九溪云服务</span>
				</NavigationMenuItem>
				...
				<NavigationMenuItem>
					{state.token ? (
						<span onClick={handleLogout} className="text-white text-lg ml-6 cursor-pointer">
							退出
						</span>
					) : (
						<span onClick={() => router.push('/login')} className="text-white text-lg ml-6 cursor-pointer">
							登录
						</span>
					)}
				</NavigationMenuItem>
