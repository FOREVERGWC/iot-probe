	...
	return (
		<main className="min-h-screen p-12">
			<div className="mb-8">
				...
				<Button
					variant="ghost"
					onClick={() => serviceAgreement?.file_path && window.open(serviceAgreement.file_path, '_blank')}>
					<Download className="h-4 w-4" />
					<span className="ml-2">服务协议</span>
				</Button>
				<Button
					variant="ghost"
					onClick={() => productDescription?.file_path && window.open(productDescription.file_path, '_blank')}>
					<Download className="h-4 w-4" />
						...
						<Card key={device.device_id} className="w-full max-w-sm mx-auto">
							<Link href={`/device/${device.device_id}`}>
								...
								<CardContent className="space-y-2">
									...
									<Row label="电源状态">
										<PowerStatus isOnline={device.is_online} electric={device.device_log?.electric} />
									</Row>
